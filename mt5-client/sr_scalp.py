"""sr_scalp.py — S/R konfluensi versi SCALPING XAUUSD (ide Ben: bias M15, entry M1).
Bias  : M15 EMA50. Konfluensi cluster >=3 dari {M15,M10,M5,M1}. Trigger rejection di bar M1.
ATR M1, SL di balik cluster -0.5ATR, TP=RMULT*R, no wick-filter. spread riil (median 0.2 poin).
IS/OOS + walk-forward 6 window. 60d M1 (~60k bar). risk 0.5%."""
...
NEAR, CLUST, ENTRYTOL, SLBUF, MINTF, RMULT = 1.2, 0.30, 0.5, 0.5, 3, 2.0
COOLDOWN = 30      # bar M1 (30 menit) jeda antar entry
PIVL, DAYS = 4, 60
CONF = ["M15", "M10", "M5", "M1"]
...
def runtrades(sym):
    ...
    m1 = fetch(sym, "M1", start, end)
    if len(m1) < 5000: return None, None
    ...
    m15 = fetch(sym, "M15", start, end); m15e = ema([b['close'] for b in m15], 50)
    conf = {tf: pivots(fetch(sym, tf, start, end) if tf != "M1" else m1) for tf in CONF}
    atr = atr_series(m1, ATRP)
    ...
    def cluster(price, tn, tc):
        ...
        return best
    pm = 0; bal = BAL; T = []; pos = None; leb = -10 ** 9; lz = None
    for i in range(60, len(m1)):
        b = m1[i]; t = b['time']
        if pos:
            hit = None
            if pos['side'] == 'buy':
        ...
        while pm + 1 < len(m15) and m15[pm + 1]['time'] <= t: pm += 1
        if math.isnan(m15e[pm]): continue
        bias = 1 if m15[pm]['close'] > m15e[pm] else -1
        price = b['close']; cl = cluster(price, NEAR * a, CLUST * a)
        if cl is None: continue
        ctr, clo, chi = cl
        if abs(ctr - price) > ENTRYTOL * a: continue
        if lz is not None and abs(ctr - lz) < 0.6 * a and i - leb < 120: continue
        rng = b['high'] - b['low']
        if rng <= 0: continue
        if bias == 1:
...
if __name__ == "__main__":
    ...
    print(f"SCALP M1 {sym} | bias M15 | konfluensi>=3/{{{','.join(CONF)}}} | RMULT{RMULT} cooldown{COOLDOWN}m | {DAYS}d")
    print("=" * 70)
    T, span = runtrades(sym)
    if not T: print("data/ trade kurang"); sys.exit()
    ...
    print(f">> profit {pc}/{N} window {'✅' if pc>=N-1 else ('⚠️' if pc>=N//2 else '❌')}")
