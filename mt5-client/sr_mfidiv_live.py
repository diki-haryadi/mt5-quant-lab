"""sr_mfidiv_live.py — FORWARD-TEST live S/R konfluensi SWING + MFI divergence (div_L15, PF~2.0 6/6).
Magic 770012. Bias D1+H4 EMA50 (searah), konfluensi >=3/{H2,H1,M30,M15}, entry/rejection M15,
divergence MFI(14): BUY=harga lower-low tembus support+MFI higher-low; SELL=mirror.
SL balik cluster -0.5ATR(M15), TP=RMULT*R. 1 pos/magic + cooldown16. risk 0.5%. notif WA+push + SL-study."""
...
MAGIC, NAME, ENTRY = 770012, "SRCONFDIV", "M15"
BIAS = ["D1", "H4"]; CONF = ["H2", "H1", "M30", "M15"]
NEAR, CLUST, ENTRYTOL, SLBUF, MINTF, RMULT, ATRP = 1.2, 0.30, 0.5, 0.5, 3, 2.0, 14
MFIP, LOOK, GAP, TOL = 14, 15, 3, 0.10
COOLDOWN_S = 16 * 900   # 16 bar M15 = 4 jam
STUDY_FILE = "/opt/mt5-quant/data/srdiv_sl_study.jsonl"; STUDY_BARS = 24
...
def study(api, sym):
    ...
    deals.sort(key=lambda d: d.get("time", 0)); done = set()
    ...
    for o, c in trades:
        ...
        et = c.get("time", 0)
        if now - et < STUDY_BARS * 900: continue
        ...
        with open(STUDY_FILE, "a") as f:
            f.write(json.dumps(dict(id=cid, exit_t=dt.datetime.fromtimestamp(et).isoformat(), side=side,
                    entry=round(entry, 3), sl=round(slx, 3), R=round(R, 3), mfe_R=round(mfe / R, 2),
                    hit_tp=mfe / R >= 2.0, profit=round(c.get('profit', 0), 2))) + "\n")

def step(api, sym, cfg, state):
    digits = cfg["digits"]
    em = api.bars(sym, ENTRY, 360); closed = em[:-1]
    if len(closed) < 120: return
    ...
    mfi = mfi_series(closed); mf = mfi[i]
    if math.isnan(mf): return
    lows = [x['low'] for x in closed]; highs = [x['high'] for x in closed]
    # bias D1+H4 searah
    dirs = []
    for tf in BIAS:
    ...
    lv = {tf: pivots(closed if tf == ENTRY else api.bars(sym, tf, 360)[:-1]) for tf in CONF}
    price = b['close']; cl = cluster(lv, price, NEAR * a, CLUST * a)
    bs = {1: "BULL", -1: "BEAR", 0: "netral"}[bias]
    log(f"bar {dt.datetime.fromtimestamp(last_t):%m-%d %H:%M} close={price:.{digits}f} ATR={a:.{digits}f} MFI={mf:.0f} bias={bs} cluster={f'{cl[0]:.{digits}f}({cl[3]}TF)' if cl else 'none'}")
    ...
    if bias == 1:
        ...
        k = min(range(lo_i, hi_i), key=lambda x: lows[x])
        if math.isnan(mfi[k]) or not (b['low'] <= lows[k] + TOL * a and mf > mfi[k]):
            log(f"  ✗ no bullish-div (MFI now {mf:.0f} vs prior {mfi[k]:.0f})"); return
        side = "buy"; sl_lvl = clo - SLBUF * a; divtxt = f"bullDIV MFI {mfi[k]:.0f}→{mf:.0f}"
    else:
        if not ((b['high'] >= clo - ENTRYTOL * a) and (b['close'] < b['open']) and (b['close'] < ctr)): return
        k = max(range(lo_i, hi_i), key=lambda x: highs[x])
        if math.isnan(mfi[k]) or not (b['high'] >= highs[k] - TOL * a and mf < mfi[k]):
            log(f"  ✗ no bearish-div (MFI now {mf:.0f} vs prior {mfi[k]:.0f})"); return
        side = "sell"; sl_lvl = chi + SLBUF * a; divtxt = f"bearDIV MFI {mfi[k]:.0f}→{mf:.0f}"
    ...
    log(f"➤ SINYAL {side.upper()} {sym} @ {entry:.{digits}f} | SL {sl} TP {tp} | {score}TF {divtxt} | lot {lot} risk ${est:.2f}")
...
def main():
    ...
    log(f"{NAME} forward-test | {a.symbol} {ENTRY} | magic {MAGIC} | div_L{LOOK} | {'🔴 LIVE demo' if a.live else '🟢 PAPER'} | risk {a.risk}%")
